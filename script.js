...
/* ══════ LOGIKA TAMBAH & HAPUS BARIS DINAMIS (MAKSIMAL 10) ══════ */
const containerInputs = $('#container-inputs');
const btnTambahBaris = $('#btn-tambah-baris');

btnTambahBaris.addEventListener('click', () => {
  const jumlahBaris = containerInputs.querySelectorAll('.baris-input-group').length;
  if(jumlahBaris >= 10){
    alert('Maksimal hanya bisa menambah 10 orang dalam 1 kali kirim!');
    return;
  }

  const divBaru = document.createElement('div');
  divBaru.className = 'baris-input-group';
  divBaru.style.cssText = 'display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center';
  divBaru.innerHTML = `
    <input type="text" class="input-nama-order" placeholder="Contoh: Nama - Jumlah Order" required>
    <button type="button" class="btn-hapus-baris" style="background:#fdeaf0;border:1px solid #f6cddb;color:#a9526f;border-radius:12px;padding:12px;cursor:pointer">✕</button>
  `;
  containerInputs.appendChild(divBaru);

  updateTombolHapusBaris();
});

containerInputs.addEventListener('click', e => {
  if(e.target.classList.contains('btn-hapus-baris')){
    e.target.closest('.baris-input-group').remove();
    updateTombolHapusBaris();
  }
});

function updateTombolHapusBaris(){
  const barisList = containerInputs.querySelectorAll('.baris-input-group');
  barisList.forEach((baris, idx) => {
    const btnHapus = baris.querySelector('.btn-hapus-baris');
    // Jika cuma sisa 1 baris, tombol hapus disembunyikan
...
/* ══════ SUBMIT MULTI INPUT (INSTANT UI UPDATE) ══════ */
$('#form-input').addEventListener('submit', e => {
  e.preventDefault();
  const st = $('#i-status');
  const inputFields = containerInputs.querySelectorAll('.input-nama-order');
  ...
  if(!listBaru.length) return status(st, 'Minimal isi 1 nama order.', 'err');

  // Buat objek array item baru
  const itemsBaru = listBaru.map((detail, index) => ({
    action: 'ADD',
  ...
  containerInputs.innerHTML = `
    <div class="baris-input-group" style="display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center">
      <input type="text" class="input-nama-order" placeholder="Contoh: Anisa - 2 Order" required>
      <button type="button" class="btn-hapus-baris" style="display:none;background:#fdeaf0;border:1px solid #f6cddb;color:#a9526f;border-radius:12px;padding:12px;cursor:pointer">✕</button>
    </div>
  `;

  status(st, `Berhasil menambahkan ${itemsBaru.length} antrian sekaligus! ♡`, 'ok');

  // Kirim ke Spreadsheet secara senyap di background
